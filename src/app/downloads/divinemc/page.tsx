...
async function getBuilds() {
  ...
  const resProject = await fetch(`${baseUrl}/api/v4/divinemc`, { cache: 'no-store' });
  const projectData = await resProject.json();
  const versions: string[] = projectData.versions || [];
  const buildsByVersion: Record<string, any[]> = {};

  for (const version of versions) {
    const resBuilds = await fetch(`${baseUrl}/api/v4/divinemc/${version}?detailed=true`, { cache: 'no-store' });
    const data = await resBuilds.json();
    const builds = data.builds.all;
    builds.forEach((build: any) => {
      build.downloadUrl = `${baseUrl}/api/v4/divinemc/${version}/${build.name}/download`;
    });
    buildsByVersion[version] = builds.sort((a: any, b: any) => Number(b.name) - Number(a.name));
  }

  return buildsByVersion;
}

export default async function DownloadsPage() {
  const buildsByVersion = await getBuilds();

  const sortedVersions = Object.keys(buildsByVersion).sort().reverse();
  const latestBuild = sortedVersions.length > 0 ? buildsByVersion[sortedVersions[0]][0] : null;
      ...
      <article className='mx-auto max-w-7xl px-6 py-16 pb-32 sm:px-8 sm:py-20 sm:pb-40 lg:px-12'>
        <header className='mx-auto max-w-3xl text-center'>
          ...
          <p className='mt-4 text-neutral-300 sm:text-lg'>
            Get the latest builds of DivineMC for your Minecraft server.
          </p>
            ...
            <div className='flex justify-center gap-4'>
              <Button size='lg' asChild className='mt-6 sm:mt-8'>
                <a
                  ...
                  aria-label={`Download latest DivineMC build for ${sortedVersions[0]}`}
                >
              ...
              <Button size='lg' asChild variant='secondary' className='mt-6 sm:mt-8'>
                <a
                  href={siteConfig.links.projects.divinemc}
                  target='_blank'
