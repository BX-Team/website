...
export function Navbar(props: HTMLAttributes<HTMLElement>) {
  ...
  return (
    <NavigationMenu value={value} onValueChange={setValue} asChild>
      ...
      >
        <NavigationMenuList className='flex h-14 w-full flex-row items-center px-4 lg:h-12' asChild>
...
export function NavbarMenuContent(props: NavigationMenuContentProps) {
  return (
    <NavigationMenuContent
      {...props}
      className={cn('grid grid-cols-1 gap-3 px-4 pb-4 md:grid-cols-2 lg:grid-cols-3', props.className)}
    >
      {props.children}
    </NavigationMenuContent>
  );
