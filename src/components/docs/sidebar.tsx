...
const itemVariants = cva(
  'text-fd-muted-foreground relative flex flex-row items-center gap-2 rounded-md p-2 text-start [overflow-wrap:anywhere] md:py-1.5 [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      active: {
        true: 'bg-fd-primary/10 text-fd-primary',
        false: 'hover:bg-fd-accent/50 hover:text-fd-accent-foreground/80 transition-colors hover:transition-none',
      },
    },
  },
);
...
export function SidebarSeparator(props: HTMLAttributes<HTMLParagraphElement>) {
  ...
  return (
    <p
      {...props}
      className={cn(
        'mb-2 inline-flex items-center gap-2 px-2 text-sm font-medium [&_svg]:size-4 [&_svg]:shrink-0',
        props.className,
      )}
      style={{
