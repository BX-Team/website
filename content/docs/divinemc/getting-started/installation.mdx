---
icon: Download
title: Installation
description: Learn how to install and set up DivineMC on your machine.
---

## Requirements

To run DivineMC, you'll need:

- [Java 21](https://adoptium.net/en-GB/temurin/releases/?version=21) or newer
- At least 4GB of RAM (recommended)
- A stable internet connection

## Installation

DivineMC follows a similar installation process to other PaperMC-based server software, making it familiar for experienced server administrators.

### Downloading the Server

<Steps>
  <Step>
    Visit our [GitHub releases page](https://github.com/BX-Team/DivineMC/releases) or [MCJars](https://mcjars.app/DIVINEMC/versions) to download the latest version of DivineMC.
  </Step>
  
  <Step>
    Make a new folder to keep your server files organized and easily accessible.
  </Step>

  <Step>
    Move the downloaded DivineMC jar file into your server directory.
  </Step>
</Steps>

:::info

We also have a SE version of DivineMC, which have a new native math optimization system that uses FFM API from Java 22. All builds are available [here](https://github.com/BX-Team/DivineMC/actions?query=branch%3Aver%2F1.21.4-se). 

**P.S. SE version requires Java 22 or newer.**

:::

### Running the Server

<Steps>
  <Step>
    Create a startup script for your operating system:

    ```bash tab="Windows" title="start.bat"
    java -Xms4096M -Xmx4096M --add-modules=jdk.incubator.vector -jar server.jar --nogui # [!code word:server.jar]
    ```

    ```bash tab="Linux/macOS" title="start.sh"
    #!/usr/bin/env bash
    java -Xms4096M -Xmx4096M --add-modules=jdk.incubator.vector -jar server.jar --nogui # [!code word:server.jar]
    ```
    
    :::info
    
    For optimal performance, get customized JVM flags from [Birdflop Flags Generator](https://www.birdflop.com/resources/flags/) based on your server specs.
    
    :::
  </Step>

  <Step>
    The first launch creates an `eula.txt` file. Open it and change the following line:

    ```bash title="eula.txt"
    # By changing the setting below to TRUE, you agree to our EULA (https://aka.ms/MinecraftEULA).
    # Thu Jan 1 00:00:00 UTC 1970
    eula=false # [!code --]
    eula=true # [!code ++]
    ```
  </Step>

  <Step>
    Run the server again using the startup script you created. Once it fully loads, your installation is complete!
  </Step>
</Steps>

## Next Steps

Now that you have your DivineMC server running, you can start [adding plugins](/docs/divinemc/getting-started/plugins) and configuring it to your liking.
