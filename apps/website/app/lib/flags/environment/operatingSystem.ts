...
type NixScript = Generate<AvailableConfig | 'existingFlags', GenerateNixResult>;
...
export const operatingSystem: EnvironmentOptions<OperatingSystemOption> = {
  ...
  pterodactyl: {
    ...
    generate: config => {
      ...
      const flagsList = [...config.existingFlags, '-Dterminal.jline=false', '-Dterminal.ansi=true'];

      const java = getJava({
        ...config,
        existingFlags: flagsList,
        fileName,
        memory,
      });
