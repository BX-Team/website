...
export async function loader(_args: Route.LoaderArgs) {
  ...
  try {
    const contributorsData = await Promise.all(
      repos.map(repo => fetchFromGitHub(`/repos/BX-Team/${repo}/contributors`)),
    );

    const contributors = (
      Object.values(
        contributorsData.flat().reduce(
          (acc: Record<string, Contributor & { contributions: number }>, contributor: Contributor) => {
...
export function meta(_args: unknown) {
  return [
    { title: 'Team | BX Team' },
    { name: 'description', content: 'Meet the BX Team developers and contributors.' },
  ];
}

function TeamMemberCard({ name, role, github }: TeamMember) {
  const content = (
        ...
        <div className='mx-auto max-w-7xl px-6 py-16 pb-32 sm:px-8 sm:py-20 sm:pb-40 lg:px-12'>
          ...
          {Object.entries(TEAM_MEMBERS).map(
            ([teamName, { description, members }]) =>
              members.length > 0 && (
                <section key={teamName} className='mt-12 sm:mt-16'>
                  <h2 className='text-2xl font-semibold text-white'>{teamName}</h2>
