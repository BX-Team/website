...
import type { Route } from './+types/downloads.$project';
...
export async function loader({ params }: Route.LoaderArgs) {
  ...
  try {
    ...
    const versions = getAllVersions(projectData.version_groups);
    ...
    return { projectData, versions, versionsMetadata, latestBuild, projectId };
  } catch {
...
export function meta({ data }: Route.MetaArgs) {
  if (!data) return [{ title: 'Project Not Found | BX Team' }];
  return [
    { title: `${data.projectData.project.name} Downloads | BX Team` },
    { name: 'description', content: `Download the latest ${data.projectData.project.name} builds` },
  ];
}

export default function ProjectDownloadsPage() {
  const { projectData, versions, versionsMetadata, latestBuild, projectId } = useLoaderData<typeof loader>();
