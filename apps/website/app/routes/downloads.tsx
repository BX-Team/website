...
export async function loader(_args: Route.LoaderArgs) {
  try {
    ...
    return { projects: projectsWithBuilds, error: null };
  } catch (err) {
    return { projects: [], error: err instanceof Error ? err.message : 'Failed to load projects' };
  }
}
...
export function meta(_args: unknown) {
  return [
    ...
    { name: 'description', content: 'Download the latest builds of our Minecraft server software' },
  ];
}
                  ...
                  >
                    <div className='p-6 pb-4'>
                      <div className='flex items-start justify-between mb-3'>
                        <div className='flex-1'>
                          <Link
                            to={`/downloads/${project.project.id}`}
                    ...
                    <div className='px-6 pb-6 pt-4 border-t border-neutral-800/50 flex gap-3'>
                      ...
                      <Button asChild color='secondary' size='default' className='cursor-pointer flex-1'>
                        <Link to={`/downloads/${project.project.id}`}>
